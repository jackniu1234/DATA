...
# @category jackniu
# @toolbar
# @menupath Tools.Jump Table Analysis
# @keybinding ctrl J
...
from ghidra.program.model.block import BasicBlockModel, CodeBlockReference, CodeBlock
import copy
from ghidra.app.decompiler import DecompileOptions, DecompInterface
from ghidra.program.model.address import AddressSpace, AddressSet, Address, AddressRange
from ghidra.program.model.lang import Register
from ghidra.program.model.mem import MemoryBlock
from ghidra.program.model.pcode import PcodeOp, Varnode, HighFunction, VarnodeAST, PcodeBlockBasic, JumpTable
...
project_dir = r'/home/jackniu/scripts/sjas-O0'

DECOMPILE_STYLE = 'firstpass'
ct_path = os.path.join(project_dir, 'callind.csv')  # callind table path
...
fp_input = open(os.path.join(home_dir, program_name+'_'+'gt.csv'))
fp_result = open(os.path.join(home_dir, 'check.txt'), 'w')
fp_final = open(os.path.join(home_dir, program_name+'_'+'result.txt'), 'w')
fp_error = open(os.path.join(project_dir, "error.txt"), 'a')
...
class PointerRepresent(object):
    ...
    __slots__ = ('base', '_hash_cache', '_bottom_cache', '_depth_cache')
    ...
    def have_base_interact_this(self, old):
        pr = self
        while not PR_interact(pr, old):
...
class TryGetLiteral(object):
    ...
    def expand_to_array(self, base, pr):
        ...
        if pr in ArrayPR and ArrayPR[pr]:
            stride = ArrayPR[pr]
            return [base + stride * i for i in range(0, 50)]
        else:
            return base
    ...
    def complexVersionImpl(self, pr):
        ...
        if isinstance(base_result, long):
            if isinstance(pr, PlusPR) and pr.offset.isConstant():
                ...
                return [base_result + WordSize * i for i in range(0, 50)]
...
class PlusPR(PointerRepresent):
    ...
    def interact(self, other):
        ...
        # 数组带来的interact, 可考虑offset，目前处理比较暴力
        if other in ArrayPR:
            return self.have_base_equal_this(other)
        ...
        # TotalUnknown为通配符
        if self.offset.isTotalUnknown():
            if isinstance(other, PlusPR):
                return PR_interact(self.base, other.base) or PR_interact(self.base, other)
            else:
                return PR_interact(self.base, other)
...
class DerefPR(PointerRepresent):
    ...
    def interact(self, other):
        ...
        if isinstance(self.base, PlusPR):
            pr = self.strip_deref_plus()
            if pr in ListPR and pr == other.strip_deref_plus():
                return True
...
class OffsetRepresent(object):
    CONSTANT, TOTALUNKNOWN, MULTSTRIDE, OTHERPOINTER = 0, 1, 2, 3
    ...
    @classmethod
    def make_mult_stride_offset(cls, index, stride):
        if is_unexpected_big_data(stride):
            stride = unsigned_to_signed(stride)
        return cls(cls.MULTSTRIDE, index, stride)

    @classmethod
    def make_other_pointer_offset(cls, pointer):
        return cls(cls.OTHERPOINTER, pointer)
    ...
    def isOtherPointer(self):
        return self.ty == self.OTHERPOINTER

class LiteralOffset(OffsetRepresent):
    def __init__(self, ty, literal=None):
        OffsetRepresent.__init__(self, ty)
    ...
    def __eq__(self, other):
        if not isinstance(other, LiteralOffset):
            return False
...
class StrideOffset(OffsetRepresent):
    def __init__(self, ty, index, stride):
        OffsetRepresent.__init__(self, ty)
        self.index = index
        self.stride = stride

    def __str__(self):
        return '+{}*{}'.format(processVarnodeFormat(self.index), hex(self.stride)[2:-1])

    def __eq__(self, other):
        return isinstance(other, StrideOffset) and self.stride == other.stride

    def __hash__(self):
        return hash(self.stride) ^ hash(StrideOffset)

class OtherPointerOffset(OffsetRepresent):
    def __init__(self, ty, pointer):
        OffsetRepresent.__init__(self, ty)
        self.pointer = pointer

    def __str__(self):
        return '+' + processVarnodeFormat(self.pointer)

    def __eq__(self, other):
        return isinstance(other, OtherPointerOffset) and other.pointer == self.pointer
...
class PRSet(object):
    ...
    def addToWorklist(self, delta):
        ...
        if not delta:
            return
        # 限制符号深度，防止无限增长
        filtered_delta = {pr for pr in delta if pr.depth < 8}
        if filtered_delta:
...
class BaseAndPath(object):
    ...
    def has_this_subset(self, other):
        """
...
class PointerAnalysis:
    ...
    param_addresses = [storage.getRegister().getAddress() for storage in
                       cc.getPotentialInputRegisterStorage(currentProgram)][-6:]

    no_pointer_def_ops = [PcodeOp.PIECE, PcodeOp.INT_EQUAL, PcodeOp.INT_NOTEQUAL,
                          PcodeOp.INT_LESS, PcodeOp.INT_SLESS, PcodeOp.INT_SLESSEQUAL, PcodeOp.INT_LESSEQUAL,
                          PcodeOp.INT_CARRY, PcodeOp.INT_SCARRY,
                          PcodeOp.INT_SBORROW, PcodeOp.INT_2COMP, PcodeOp.INT_NEGATE, PcodeOp.INT_XOR,
                          PcodeOp.INT_OR, PcodeOp.INT_RIGHT, PcodeOp.INT_LEFT, PcodeOp.INT_SRIGHT,
    ...
    no_pointer_use_ops = [PcodeOp.PIECE, PcodeOp.INT_SLESS, PcodeOp.INT_SLESSEQUAL,
                          PcodeOp.INT_CARRY, PcodeOp.INT_SCARRY,
                          PcodeOp.INT_SBORROW, PcodeOp.INT_2COMP, PcodeOp.INT_NEGATE, PcodeOp.INT_XOR,
                          PcodeOp.INT_OR, PcodeOp.INT_RIGHT, PcodeOp.INT_LEFT, PcodeOp.INT_SRIGHT,
                          PcodeOp.INT_MULT, PcodeOp.INT_DIV, PcodeOp.INT_SDIV, PcodeOp.INT_REM, PcodeOp.INT_SREM,
                          PcodeOp.BOOL_XOR, PcodeOp.BOOL_AND, PcodeOp.BOOL_OR]  # 结果一定不是指针的pcodes
    sys_function_names = ['MAYALIAS', 'NOALIAS', 'MUSTALIAS', 'EXPECTEDFAIL_MAYALIAS']  # 不进入过程间搜索的函数

    def table_size_match(self, tb, gt_tb):
        return tb and (tb == gt_tb)
    ...
    def print_function(self, func, fp):
        ...
        if func not in self.func_pcodes_map:
    ...
    def run(self):
        ...
        # self.print_function(getFunctionContaining(currentAddress), fp_print)
        ...
        solved_cnt = 0
        faileds = []
        todolist = []  # 用于存储需要重试的任务

        # 定义处理单行数据的内部函数，复用逻辑
        def process_row(row, is_retry):
            self.currentAddress, gt_base, gt_entry_size, gt_table_size = (
                toAddr(row[0]), int(row[1], 16), int(row[2]), int(row[3]))

            try:
                # 如果失败会抛出异常
                result = self.solve_task4()

                base, entry_size, table_size, prs, jmp_targets = result

                # 写结果，出发点作为第一列，后续为跳转地址
                print >> fp_final, hex(self.currentAddress.getOffset())[2:-1],
                for jmp_target in jmp_targets:
                    print >> fp_final, hex(jmp_target)[2:-1],
                print >> fp_final, ''

                # 向终端打印结果
                if base != gt_base or entry_size != gt_entry_size or not self.table_size_match(table_size,
                                                                                               gt_table_size):
                    faileds.append(str(self.currentAddress))
                    print >> fp_result, "{},{},{},{}".format(self.currentAddress, hex(base)[2:-1], entry_size,
                                                             table_size)

                    print >> fp_error, "{} : {}".format(program_name, self.currentAddress)
                    print str(self.currentAddress)[2:], ': ',
                    for pr in prs:
                        print '\t', pr,
                    print '',
                    print '\nbound: {}\n'.format(table_size)
                ...
                # 不采用Ghidra的结果, 设为0
                if USE_GHIDRA:
                    jmp_targets = self.get_computed_jump_targets(self.currentAddress)
                    for jmp_target in jmp_targets:
                        print >> fp_final, hex(jmp_target)[2:-1],
                print >> fp_final, ''

                faileds.append(str(self.currentAddress))
                print "exception occurs: {} : {}".format(program_name, self.currentAddress)
                traceback.print_exc()
                print >> fp_error, program_name, ' ', str(self.currentAddress)[2:], ' exception happen'
                return False

        # 读取CSV并处理第一遍
        reader = csv.reader(fp_input)
        next(reader)
        for row in reader:
            res = process_row(row, is_retry=False)
            if res:
                solved_cnt += 1
            all_cnt += 1
        # 处理 todolist 中的任务 (触发 solve_task4 的 retry 逻辑)
        if todolist:
            # 为这些函数重新生成pcode和摘要
            for func in self.control_flow_added_funcs:
                self.recreate_func(func)

            # 重新生成反编译器的接口
            self.decompInterface = setup_decompiler()

            for row in todolist:
                res = process_row(row, is_retry=True)
                if res:
                    solved_cnt += 1

        print 'all: {} solved: {}'.format(all_cnt, solved_cnt)

    def recreate_func(self, func):

        # 重新生成该函数的摘要
        del self.func_pcodes_map[func]
        del self.func_summary_map[func]

    def is_legal_pr1(self, pr):
        bottom = pr.get_bottom_base()
        return (isinstance(pr, DerefPR) and pr.depth == 3 and isinstance(pr.base, PlusPR) and
                pr.base.offset.isMultStride() and isinstance(bottom, ConstantPR))

    def is_legal_pr2(self, pr):
        bottom = pr.get_bottom_base()
        return (isinstance(pr, PlusPR) and pr.depth == 2 and isinstance(pr.offset, OtherPointerOffset)
                and isinstance(bottom, ConstantPR))

    def analysisPR(self, M, pointer, func, jmp_op):
        # 通常跳表对应的简介跳转只有一个pr
        for pr in M[pointer]:
            if self.is_legal_pr1(pr):
                assert isinstance(pr, DerefPR) and isinstance(pr.base, PlusPR) and isinstance(pr.base.offset, StrideOffset)
                base = pr.base.get_bottom_base().base
                entry_size = pr.base.offset.stride
                table_size = self.find_bound(pr.base.offset.index, func, jmp_op)

                return 1, base, entry_size, table_size

            elif self.is_legal_pr2(pr):
                assert isinstance(pr, PlusPR) and isinstance(pr.offset, OtherPointerOffset)
                base = pr.base.get_bottom_base().base
                ptr = pr.offset.pointer

                for offset_pr in M[ptr]:
                    if self.is_legal_pr1(offset_pr):
                        assert isinstance(offset_pr, DerefPR) and isinstance(offset_pr.base, PlusPR) and isinstance(offset_pr.base.offset, StrideOffset)
                        entry_size = offset_pr.base.offset.stride
                        table_size = self.find_bound(offset_pr.base.offset.index, func, jmp_op)
                        return 2, base, entry_size, table_size

        return None

    def get_post_execution_address_set(self, pcode_op):
        """
        Args:
            pcode_op (PcodeOp): 目标 Pcode 操作
        Returns:
            AddressSet: 包含所有在逻辑上可能在 pcode_op 之后的地址集合
        """
        program = currentProgram
        block_model = BasicBlockModel(program)

        # 初始化结果集合
        result_set = AddressSet()

        # 1. 获取 PcodeOp 对应的指令地址和所在的 Basic Block
        op_addr = pcode_op.getSeqnum().getTarget()
        target_block = block_model.getCodeBlockAt(op_addr, monitor)

        if target_block is None:
            print("Error: Could not find basic block for address {}".format(op_addr))
            return result_set

        # 2. 获取所有后继 (Descendants) - target 能到达的块
        descendants = set()
        queue = [target_block]
        visited_fwd = {target_block}

        while queue:
            curr = queue.pop(0)
            dests = curr.getDestinations(monitor)
            while dests.hasNext():
                dest_ref = dests.next()
                dest_block = dest_ref.getDestinationBlock()

                if dest_block not in visited_fwd:
                    visited_fwd.add(dest_block)
                    descendants.add(dest_block)
                    queue.append(dest_block)

        # 3. 处理其他 Basic Blocks
        # 逻辑：是后继，且不是 target 自己
        for block in descendants:
            if block not in descendants and block != target_block:
                result_set.add(block.getMinAddress(), block.getMaxAddress())

        # 5. 处理 Target Block 内部 (Split Logic)
        # 我们只包含 target_block 中，地址严格小于 op_addr 的部分
        block_end = target_block.getMaxAddress()

        # 比较地址：如果 op_addr 大于 block_start，说明 block 内部有指令在 op 之前
        if op_addr.compareTo(block_end) < 0:
            # 添加范围 [BlockStart, OpAddress - 1]
            # subtract(1) 获取前一个字节地址，AddressSet 会自动处理范围闭合
            result_set.add(op_addr, block_end)

        return result_set


    def getAlias(self, variable, summary):
        M, RM, gprs = summary.M, summary.RM, summary.globalPRs

        ans = []
        if variable in M and len(M[variable]):
            prset = M[variable]
            for alias in RM[prset]:
                if alias not in ans:
                    ans.append(alias)

        return ans

    def getAliasAfterLoad(self, addr, summary):
        M, RM, gprs = summary.M, summary.RM, summary.globalPRs
        ...
        if addr in M:
            for pr in M[addr]:
                ...
                if load_pr in gprs:
                    for alias in RM[gprs[load_pr]]:
                        if alias not in ans:
                            ans.append(alias)
        return ans

    def interCheckForConstant(self, callee, varnode):
        """ 目前过程间分析只考虑如果Varnode是形参 """
        # 解析slot
        slot = -1
        for i, ssa_param in enumerate(self.func_ssa_params_map[callee]):
            if ssa_param and varnode.getAddress() == ssa_param.getAddress():
                slot = i + 1
                break
        if slot == -1:
            return None
        ...
        def dfs_for_constant(var):
            ...
            _def = var.getDef()
            if _def and self.is_mov_op(_def):
                if _def.getInput(0).isConstant():
                    return _def.getInput(0).getOffset()
                else:
                    return dfs_for_constant(_def.getInput(0))

            elif _def and _def.getOpcode() == PcodeOp.MULTIEQUAL:
                max_const = 0
                for i in range(_def.getNumInputs()):
                    inputi = _def.getInput(i)
                    const = dfs_for_constant(inputi)
                    if const:
        ...
        max_const = 0
        callers = self.myGetCallingFunctions(callee)
        for caller in callers:
            self.decompile_function(caller)
            call_sites = self.get_all_call_sites(caller, callee)
            for call_site in call_sites:
                arg_varnode = call_site.getInput(slot)
                const = dfs_for_constant(arg_varnode)
                if const and const > max_const:
                    max_const = const
    ...
    def getConstraintsv1(self, variable, func, summary):
        ops = self.get_pcodes(func)

        constant_sizes = []
        corresponding_ops = []

        inter_max_const = [0]  # silly python2

        vis = set()
        # 寻找v的等价节点，与v应用的cmp和and指令
        # 等价包括：mov指令、phi指令、LOAD指令、EQUAL指令
        def dfs(v):
            if v not in vis:
                vis.add(v)
            else:
            ...
            def process_alias(addr, summary):
                """
                 当index通过Store存入内存或者是通过LOAD读取自内存，需要处理别名的问题
                 由于index本身是个数值，所以在设计上我们没有把index推测为指针变量，因此，通常需要查看addr的别名
                 但是在index是内存变量等场景下，index可能也被推测为了指针，需要查看deref(addr)的别名
                 如果addr没有出现在M中，说明我们做的快速迭代的分析可能没有将其识别为指针或者由于PRSet的size设置的过低导致，PR没有传播过来；
                 此时根据pattern尝试进行复原
                 :type addr Varnode
                ...
                # 尝试使用别名分析解决
                # part1: 寻找addr的别名
                for addr_alias in self.getAlias(addr, summary):
                    uses = addr_alias.getDescendants()
                    if uses:
                        for use in addr_alias.getDescendants():
                            if use.getOpcode() == PcodeOp.LOAD:
                                dfs(use.getOutput())

                # part2: 寻找deref(addr)的别名, 这种情况对应index变量本身被识别为了指针
                for alias in self.getAliasAfterLoad(addr, summary):
                    dfs(alias)

                # part3: 基于句法pattern寻找别名
                addr_def = addr.getDef()
                if addr_def and addr_def.getOpcode() == PcodeOp.INT_ADD:
                    base, offset = addr_def.getInput(0), addr_def.getInput(1).getAddress()

                    uses = base.getDescendants()
                    if uses:
                        for use in base.getDescendants():
                            if use is not addr_def and \
                                    use.getOpcode() == PcodeOp.INT_ADD and use.getInput(
                                1).getAddress() == offset:
                                addr_alias = use.getOutput()
                                for alias_use in addr_alias.getDescendants():
                                    if alias_use.getOpcode() == PcodeOp.LOAD:
                                        dfs(alias_use.getOutput())


            print __LINE__(), processVarnodeFormat(v)

            # check def
            _def = v.getDef()
            if _def:
                if self.is_mov_op(_def):
                    dfs(_def.getInput(0))
                elif _def.getOpcode() == PcodeOp.MULTIEQUAL:
                    for i in range(_def.getNumInputs()):
                        dfs(_def.getInput(i))
                elif _def.getOpcode() == PcodeOp.LOAD:
                    # index使用过程中出现了内存别名
                    addr = _def.getInput(1)
                    process_alias(addr, summary)

                elif _def.getOpcode() == PcodeOp.INT_AND:
                    if _def.getInput(0).isConstant() and not is_unexpected_big_data(_def.getInput(0).getOffset()):
                        constant_sizes.append(_def.getInput(0).getOffset() + 1)
                        corresponding_ops.append(_def)
                    elif _def.getInput(1).isConstant() and not is_unexpected_big_data(_def.getInput(0).getOffset()):
                        constant_sizes.append(_def.getInput(1).getOffset() + 1)
                        corresponding_ops.append(_def)
            else:
                # 进行一个简单的过程间分析，只check
                if self.varnode_is_formal_param(v, func):
                    const = self.interCheckForConstant(func, v)
                    if const:
                        inter_max_const[0] = max(inter_max_const[0], const)

            # check use
            uses = v.getDescendants()
            if uses:
                for use in v.getDescendants():
                    if use.getOpcode() == PcodeOp.INT_EQUAL:
                        if use.getInput(0) == v:
                            dfs(use.getInput(1))
                        else:
                            dfs(use.getInput(0))

                    elif self.is_mov_op(use):
                        dfs(use.getOutput())

                    elif use.getOpcode() == PcodeOp.MULTIEQUAL:
                        dfs(use.getOutput())

                    elif use.getOpcode() == PcodeOp.STORE and use.getSlot(v) == 2:
                        # index使用过程中出现了内存别名
                        addr = use.getInput(1)
                        process_alias(addr, summary)

                    elif use.getOpcode() == PcodeOp.INT_LESS:
                        if use.getInput(0) == v and use.getInput(1).isConstant():
                            constant_sizes.append(use.getInput(1).getOffset())
                            corresponding_ops.append(use)
                        elif use.getInput(1) == v and use.getInput(0).isConstant():
                            constant_sizes.append(use.getInput(0).getOffset() + 1)
                            corresponding_ops.append(use)
            else:
                for op in ops:
                    if op.getOpcode() == PcodeOp.INT_LESS:
                        if op.getInput(0) == v and op.getInput(1).isConstant():
                            constant_sizes.append(op.getInput(1).getOffset())
                            corresponding_ops.append(op)
                        elif op.getInput(1) == v and op.getInput(0).isConstant():
                            constant_sizes.append(op.getInput(0).getOffset() + 1)
                            corresponding_ops.append(op)
                    elif op.getOpcode() == PcodeOp.INT_AND and op.getOutput() == v:
                        if op.getInput(0).isConstant() and not is_unexpected_big_data(op.getInput(0).getOffset()):
                            constant_sizes.append(op.getInput(0).getOffset() + 1)
                            corresponding_ops.append(op)
                        elif op.getInput(1).isConstant() and not is_unexpected_big_data(op.getInput(0).getOffset()):
                            constant_sizes.append(op.getInput(1).getOffset() + 1)
                            corresponding_ops.append(op)
                    elif self.is_mov_op(op) and op.getInput(0) == v and op.getOutput() != v:
                        dfs(op.getOutput())

        dfs(variable)

        return constant_sizes, corresponding_ops, inter_max_const[0]


    def fast_bound_path(self, jmp_op):
        """ 查找bound的短路路径，检查前驱基本块中是否有一个对应的cmp指令，如果有，直接取出 """
        op_address = jmp_op.getSeqnum().getTarget()
        jmp_inst = getInstructionAt(op_address)

        reg_addr = None
        if jmp_inst.getMnemonicString() == 'JMP':
            for obj in jmp_inst.getOpObjects(0):
                if isinstance(obj, Register):
                    reg_addr = obj.getAddress()
                    break
        ...
        block = block_model.getFirstCodeBlockContaining(op_address, monitor)

        preds = []
        it = block.getSources(monitor)
        while it.hasNext():  # type: CodeBlockReference
            ref = it.next()
            source = ref.getSourceBlock()  # type: CodeBlock
            preds.append(source)

        preds.sort(key=lambda pred: pred.getFirstStartAddress().getOffset())
        for pred in preds:
            inst_iter = currentProgram.getListing().getInstructions(pred, False)  # False = backward
            for inst in inst_iter:
                if inst.getMnemonicString() == 'CMP':
                    if (isinstance(inst.getRegister(0), Register) and inst.getRegister(0).getAddress() == reg_addr
                            and isinstance(inst.getScalar(1), Scalar)):
                        return inst.getScalar(1).getValue()+1
                    elif (isinstance(inst.getRegister(1), Register) and inst.getRegister(1).getAddress() == reg_addr
                            and isinstance(inst.getScalar(0), Scalar)):
                        return inst.getScalar(0).getValue()+1

        return None

    def find_bound(self, variable, func, jmp_op):
        """
         基于深度优先搜索，遍历def-use chain，寻找所有variable等价的节点，寻找过程中会考虑内存别名
         在遍历过程中，如果发现等价节点用于了目标cmp或者and指令，则收集
         最终从所有收集的结果中，选择距离jmp op地址最近又地址较小的那个；也可选择最小的那个
        :type variable VarnodeAST
        :type func Function
        :type jmp_op PcodeOp
        """
        fast_result = self.fast_bound_path(jmp_op)
        if fast_result:
            return fast_result

        summary = self.getSummary(func)
        constant_sizes, corresponding_ops, inter_max_const = self.getConstraintsv1(variable, func, summary)

        def bfs():
            """ 从jmp_op所在的基本块出发，沿着控制流逆向进行BFS搜索，离着最近的基本块中的目标常量作为结果 """
            base = jmp_op.getParent()
            vis = set()
            que = []
            que.append(base)
            while que:
                block = que.pop(0)  # type: PcodeBlockBasic
                if block not in vis:
                    vis.add(block)
                else:
                    continue

                # 如果一个基本块施加了多个约束，通常是既通过and又通过cmp施加约束，取地址最高执行顺序最后的指令
                min_opAddr = 0
                result = None
                for i, op in enumerate(corresponding_ops):
                    if op.getParent() is block and op.getSeqnum().getTarget().getOffset() > min_opAddr:
                        min_opAddr = op.getSeqnum().getTarget().getOffset()
                        result = constant_sizes[i]
                if result:
                    return result

                # 按照地址升序排列前驱，主要观察是jmp通常与cmp ja一起应用，通过fallthrough进入，所以cmp ja通常位于低地址
                preds = []
                for i in range(block.getInSize()):
                    preds.append(block.getIn(i))
                preds.sort(key=lambda pred: pred.getStart().getOffset())
        ...
        if ans is not None:
            return ans
        elif constant_sizes and min(constant_sizes) < 256:
            return min(constant_sizes)
        elif inter_max_const:
            return inter_max_const+1
        else:
            return None

    def updateCFG(self, ty, base_literal, entry_size, table_size):
        """
        ty: 如果为1，跳转地址为*(base+i*entry_size)，否则为base+*(base+i*entry_size)
        由于有些跳转目标即使Ghidra分析出来了，但是缺失某些pass，所以反编译结果中还是缺失跳转目标对应的基本块，所以每个跳表恢复都需要强行override
        base: Address 对象，跳转表的起始地址
        entry_size: 整数，表中每个表项的大小（例如 4 或 8 字节）
        table_size: 整数，表项的数量
        """

        jmp_targets = []  # type: list[int]

        base = toAddr(base_literal)
        jmp_addr = self.currentAddress
        program = currentProgram
        ...
        for i in range(table_size):
            try:
                # 计算当前表项在内存中的地址
                current_entry_addr = base.add(i * entry_size)

                # 根据表项大小读取目标地址的值
                if entry_size == 8:
                    target_val = memory.getLong(current_entry_addr)
                    if target_val & 0x8000000000000000:
                        target_val = target_val - 0x10000000000000000  # 减去 2^64
                else:
                    target_val = memory.getInt(current_entry_addr) & 0xFFFFFFFF
                    if target_val & 0x80000000:
                        target_val = target_val - 0x100000000  # 减去 2^32

                if ty == 2:
                    target_val += base_literal
                ...
                # 获取目标地址所在的内存块
                target_block = memory.getBlock(target_addr)

                # 检查2: 内存块是否存在
                if target_block is None:
                    continue

                # 检查3: 内存块是否已初始化 (排除未初始化的段)
                if not target_block.isInitialized():
                    continue

                # 检查4: 内存块是否具有可执行权限 (这是判断是否为代码的关键)
                # 注意：某些混淆代码或特殊固件可能没有正确标记 'X'，如果通过率低可去掉此行
                if not target_block.isExecute():
                    # 也可以放宽条件：如果不可执行，检查是否至少是只读段（防止跳到堆栈/数据区）
                    continue

                if target_val not in jmp_targets:
                    jmp_targets.append(target_val)
        ...
        # 2. 只有当添加了新的边时，才重建函数
        func = listing.getFunctionContaining(jmp_addr)
        if func:
            inst = listing.getInstructionAt(jmp_addr)

            destlist = ArrayList()
            for val in jmp_targets:
                jmp_target = toAddr(val)
                destlist.add(jmp_target)
                inst.addOperandReference(0, jmp_target, RefType.COMPUTED_JUMP, SourceType.USER_DEFINED)

            jumpTable = JumpTable(jmp_addr, destlist, True)
            jumpTable.writeOverride(func)

            CreateFunctionCmd.fixupFunctionBody(currentProgram, func, monitor)

            # 在跑完第一轮之后统一进行函数重建
            if func not in self.control_flow_added_funcs:
                self.control_flow_added_funcs.append(func)

        return jmp_targets

    def get_computed_jump_targets(self, jmp_addr):
        """
        获取指定地址处间接跳转（Computed Jump）的所有目标地址。
        通常用于获取 Switch Case 的所有分支目标。

        Args:
            jmp_addr: ghidra.program.model.address.Address 对象
        ...
        for ref in references:
            ...
            # 3. 筛选引用类型
            # COMPUTED_JUMP: 标准的间接跳转 (如 JMP RAX, switch)
            # CONDITIONAL_COMPUTED_JUMP: 带条件的间接跳转 (较少见，但存在)
            if ref_type == RefType.COMPUTED_JUMP or ref_type == RefType.CONDITIONAL_COMPUTED_JUMP:
                targets.append(ref.getToAddress().getOffset())

        return targets


    def solve_task4(self):
        """ task4对应解析某个基于跳表进行间接跳转的目的地址 """
        ret = self.resolve_task4_params()
        assert ret
        ...
        res = self.analysisPR(M, pointer, func, op)
        if res:
            ty, base, entry_size, table_size = res
            if base and entry_size and table_size:
                jmp_targets = self.updateCFG(ty, base, entry_size, table_size)
                return base, entry_size, table_size, M[pointer], jmp_targets

        assert False, "can't solve"
    ...
    def setImageBaseToZero(self):
        trans_id = currentProgram.startTransaction("Set Image Base to 0")

        try:
            # 3. 设置 Image Base
            # 参数1: 新的基地址 (Address对象)
            # 参数2: commit (True 表示不仅修改基址，还会尝试重定位现有的内存块)
            currentProgram.setImageBase(toAddr(0), True)
            print("Image Base successfully set to 0.")
        except Exception as e:
            print("Error setting image base: " + str(e))
        finally:
            # 4. 提交并结束事务
    ...
    def resolve_task4_params(self):
        """
        解析currentAddress所指向的间接跳转指令对应的BRANCHIND指令
        """
        if currentProgram.getImageBase() and currentProgram.getImageBase().getOffset() == 0x100000:
            self.setImageBaseToZero()
        ...
        ops = self.get_pcodes(func)
        for op in ops:
            if (op.getSeqnum().getTarget() == addr and
                    (op.getOpcode() == PcodeOp.BRANCHIND or op.getOpcode() == PcodeOp.CALLIND)):
                print("Here get the operand of PointerAnalysis (task4) :")
                print_pcode(sys.stdout, op)
                return func, op, op.getInput(0)

        assert False, "can't find op"

    def dumpGraph(self, initial_node, fp):
        """
         以树的形式打印过程间访问图
    ...
    def decompile_function(self, func):
        ...
        # todo: 未来支持存放在栈上的参数
        for i in range(min(6, param_nums)):
    ...
    def computeEdge(self, curr_node):
        ...
        MAX_ACCESS_PATH_LEN = 6  # 最大访问路径长度（解引用+偏移的总数）
        ...
        def helper_applyOneOperation(bases, edge):
            next_prs = set()
            for pr in bases:
                if isinstance(edge, PlusEdge):
        ...
        def useStoreEdge(_prset, _new_p, _func):
            # [Heuristic] 如果路径过深，或者 new_p 导致路径变得极长，则忽略 Store Edge
            # Store Edge 通常是别名分析，容易导致状态爆炸，深层搜索时需谨慎
            if len(_new_p) + len(history_path) > MAX_ACCESS_PATH_LEN:
                return

            # 加入que与vis
            vk = self.get_vis_key(_prset, _new_p)

            if vk not in self.vis:
                self.vis.add(vk)

                _new_prs = copy.copy(_prset.PRs)
                for _edge in _new_p:
                    _new_prs = helper_applyOneOperation(_new_prs, _edge)

                self.que.append(QueNode(_func, _prset, _new_p, _new_prs,
                                        transfer_edge=Edge(Edge.STORE_EDGE), parent=curr_node))

        def buildPRs(bases, p, to_func, S):
            store_prsets = set()
            gprs = S.globalPRs
            curr_prs = {b for b in bases if b}

            for i, edge in enumerate(p):
                curr_prs = helper_applyOneOperation(curr_prs, edge)
                ...
                for pr in curr_prs:
                    if pr in gprs:
                        ...
                        flag = False
                        for delta_pr in delta:
                            if isinstance(delta_pr.get_bottom_base(), SymbolPR) \
                                    and delta_pr.get_bottom_base().ty == SymbolPR.SP:
                                continue
                            flag = True

                        # 如果prset中存在非local的pr，应用store edge
                        if flag and gprs[pr] not in store_prsets:
                            store_prsets.add(gprs[pr])
                            useStoreEdge(_prset=gprs[pr], _new_p=p[i + 1:], _func=to_func)
            return curr_prs
        ...
        #    需要分析，但是无法获得这块内存的size
        #    此外，如果某个地址解析需要跨越多次全局变量，如*(*(global1+0x4))，
        #    global1+0x4可以解析，但随后不能解析，当前的实现也无法支持
        #    此外，Ghidra恢复的xref也可能并不完备
        # ---------------------------------------------------------

        # 1. 获得引用的literal
        literals = []
        for pr in curr_prset:
            bottom = pr.get_bottom_base()
            # 如果一个pr的bottom是常量但是却无法解析，需要通过constant edge转移
            if isinstance(bottom, ConstantPR) and try_get_literal(pr) is None:
        ...
        # 2. 得到full path，与后续逻辑无关，提前处理
        # 由于Plus工厂那里对一部分offset进行了折叠，所以base_varnode与base_literal不能对应起来了
        # 这里进行了假设，即该prset只对应一个constant出发的边
        base_var, full_path = None, None
        for _base_var, local_path in pdGraph[curr_prset]:
            if _base_var.isConstant():
                base_var = _base_var
                if history_path:
        ...
        if base_var:
            ...
            for new_func in refers:
                ...
                basePR = ConstantPR(base_var.getOffset())
                if basePR in new_func_globalPRs:
                    new_prset = new_func_globalPRs[basePR]

                    vis_key = self.get_vis_key(new_prset, full_path)
                    if vis_key in self.vis:
                        continue
                    self.vis.add(vis_key)

                    # 构造新的prs，直接复用当前的PRs会触发没有必要的store edge
                    new_prs = buildPRs(new_prset.PRs, full_path, new_func, new_func_summary)

                    self.que.append(QueNode(new_func, new_prset, full_path, new_prs,
        ...
        for base_varnode, local_path in pdGraph[curr_prset]:  # type: Varnode, list
            ...
            # [Heuristic 1] 路径长度剪枝
            # 如果访问路径（解引用链）过长，说明分析精度已丢失，直接丢弃该路径
            if len(full_path) > MAX_ACCESS_PATH_LEN:
                continue
            ...
            if self.varnode_is_formal_param(base_varnode, func):
                # 解析slot
                slot = -1
                ...
                for caller in callers:
                    ...
                    for call_op in call_ops:
                        ...
                        new_prs = buildPRs(caller_prset, full_path, caller, caller_summary)
            ...
            # reverse-return-edge; caller->callee
            elif self.varnode_is_formal_ret(base_varnode, func):
                ...
                for callee in callees:
                    ...
                    for actual_ret in actual_rets:
                        ...
                        new_prs = buildPRs(callee_ret_prset, full_path, callee, callee_summary)
            ...
            for actual_prset in pdGraph:
                ...
                for act_base_varnode, act_path in pdGraph[actual_prset]:
                    ...
                    if base_and_path_obj.has_this_subset(act_base_and_path):
                        suffix = base_and_path_obj.get_suffix(act_base_and_path)

                        # 出发点去重，并且这个节点与curr_node是等价的，不需要再进行处理
                        # 检查禁忌表 (key 是 prset + path)
                        vis_key = self.get_vis_key(actual_prset, suffix)
                        if vis_key not in self.vis:
                        ...
                        for formal_param_pr in callee_to_formal_param_prs[callee]:
                            if formal_param_pr in callee_globalPRs:
                                ...
                                for suffix in valid_suffixes:
                                    ...
                                    new_prs = buildPRs([formal_param_pr], suffix, callee, callee_summary)

                                    self.que.append(QueNode(callee, formal_prset, suffix, new_prs,
                                                            transfer_edge=Edge(Edge.PARAM_EDGE), parent=curr_node))

                # return-edge; callee->caller
                elif is_ret_candidate:
                        ...
                        for formal_return_pr in formal_return_prs:
                            if formal_return_pr in caller_globalPRs:
                                ...
                                for suffix in valid_suffixes:
                                    ...
                                    new_prs = buildPRs([formal_return_pr], suffix, caller, caller_summary)

                                    self.que.append(QueNode(caller, formal_prset, suffix, new_prs,
    ...
    def ResolveCallind(self, func, pointer, opAddr):
        ...
        initialPRs = copy.copy(M[pointer].PRs)
        initial_node = QueNode(func, M[pointer], [], initialPRs, transfer_edge=None, parent=None)
        self.que.append(initial_node)
        self.vis.add(self.get_vis_key(M[pointer], []))
        ...
        self.dumpGraph(initial_node, fp_process)

        results = self.CheckPRSet(prs)
        self.query_to_results_map[pointer] = QueryResult(1, results, prs)
    ...
    def is_mov_op(self, op):
        opcode = op.getOpcode()
        if opcode in [PcodeOp.COPY, PcodeOp.INDIRECT, PcodeOp.INT_ZEXT, PcodeOp.INT_SEXT]:
            return True
        elif opcode == PcodeOp.SUBPIECE and op.getInput(1).getOffset() == 0:
            return True
        elif opcode == PcodeOp.INT_AND and is_unexpected_big_data(op.getInput(1).getOffset()):
            return True
        else:
            return False
    ...
    def InferPointerVariable(self, func):
        ...
        for op in ops:
            if op.getOpcode() == PcodeOp.LOAD:
                ...
                if not op.getInput(2).isConstant():
                    # 特殊处理，可能是no_pointers，但是要加入pointers集合中，因为它一定会有一个PR
                    pointers.add(op.getInput(2))
                    clearly_pointers.add(op.getInput(2))
        ...
        last_capacity = -1
        cnt = 0
        while len(pointers) != last_capacity and cnt < 10:  # 如果pointers集合的大小不再增长，则停止
            last_capacity = len(pointers)
            cnt += 1

            for op in ops:
                if op.getOutput() in not_pointers:
                    continue

                if self.is_mov_op(op):
                    ...
                    if op.getOutput() in pointers:
                        if self.is_special_int_add(op):
                            pointers.add(op.getInput(1))
                            if AnalysisOffset(op.getInput(0)).isTotalUnknown():
                                pointers.add(op.getInput(0))
                        else:
        ...
        # for ptr in pointers:
        #     if processVarnodeFormat(ptr).startswith('t71(8)'):
        #         assert False

        # kill逻辑；正向数据流，如果input不是指针变量，则output也不是指针变量
        last_capacity = -1
        cnt = 0
        while len(pointers) != last_capacity and cnt < 10:  # 如果pointers集合的大小不再减小，则停止
            last_capacity = len(pointers)
            cnt = cnt + 1
    ...
    def buildVariableDependencyGraph(self, pointers_set):
        ...
        for pointer in pointers_set:
            ...
            elif self.is_mov_op(the_def):
                input_var = the_def.getInput(0)
                if input_var in pointers_set:
    ...
    def cleanPRSet(self, M):
        ...
        for pointer in M:
            if M[pointer] not in vis:
                ...
                pr_to_reserved = set()
                for pr in M[pointer]:
                    bottom_base = pr.get_bottom_base()
                    if isinstance(bottom_base, SymbolPR) and bottom_base.ty == SymbolPR.SP:
                        continue
                    else:
    ...
    def EstimatePR(self, func, pointers, ptr_related_ops):
        ...
        for pointer in pointers:
            the_def = pointer.getDef()
            # to_roots为空，那么也不存在浅拷贝
            if the_def and (the_def.getOpcode() == PcodeOp.MULTIEQUAL or
                            self.is_mov_op(the_def)) and pointer_to_roots[pointer]:
                continue
            else:
        ...
        for op in ptr_related_ops:
            ...
            if opcode == PcodeOp.LOAD:
                ...
                if self.is_special_int_add(op):
                    ...
                    if offset1.isTotalUnknown():
                        if input1 in phinodes:
                            for inputi in phinodes[input1]:
                                M[inputi].addInitialEdge(PlusEdge(OtherPointerOffset.make_other_pointer_offset(input0)), M[output])
                        else:
                            M[input1].addInitialEdge(PlusEdge(OtherPointerOffset.make_other_pointer_offset(input0)), M[output])
                    else:
                        if input1 in phinodes:
                            for inputi in phinodes[input1]:
        ...
        for pointer in pointers:
            ...
            if is_register and address == self.sp_address and def_info is None:
                ...
                # print __LINE__(), toAddr(pointer.getOffset())
                # assert validAddrRange.contains(toAddr(pointer.getOffset()))
                ...
                if key in phinode_to_prset:
                    ...
                    for dep in phinodes[phinode]:
                        ...
                        if M[dep] in pdGraph:
                            ...
                            if dep not in RM[M[dep]]:
                                RM[M[dep]].append(phinode)
    ...
    def myGetCallingFunctions(self, func):
        """
         ...
         :type func Function
        """
        if func in self.func_callers_map:
            return self.func_callers_map[func]
    ...
    def myGetCalledFunctions(self, func):
        """
         ...
         :type func Function
        """
        if func in self.func_callees_map:
            return self.func_callees_map[func]
...
def is_unexpected_big_data(data):
    """对于跳表size恢复任务，认为以至少2个f开头的整数实际上是负数, 如0xffffffe8"""
    return data > 0 and hex(data).startswith('0xff')
...
class ReadAddrAndCheckClass(object):
    ...
    def __call__(self, addr):
        ...
        # 不合法的data会导致toAddr崩溃
        if data and data > 0 and data < validAddrRange.getMaxAddress().getOffset() and toAddr(data) in validAddrRange:
            result = data
        else:
            result = None
...
class OffsetAnalyzer(object):
    def __init__(self):
        self.cache = {}  # type: dict[Varnode, OffsetRepresent]

    def impl(self, varnode):

        if varnode.isConstant():
            return LiteralOffset.make_constant_offset(varnode.getOffset())
        ...
        while _def:
            if _def.getOpcode() == PcodeOp.LOAD:
                if _def.getInput(1).isConstant():
                    ...
                    if data:
                        return LiteralOffset.make_constant_offset(data)

                return LiteralOffset.make_total_unknown_offset()

            elif _def.getOpcode() == PcodeOp.INT_ZEXT or _def.getOpcode() == PcodeOp.COPY:
                input0 = _def.getInput(0)
                if input0.isConstant():
                    return LiteralOffset.make_constant_offset(input0.getOffset())
                ...
                if input1.isConstant():
                    return StrideOffset.make_mult_stride_offset(stride=input1.getOffset(), index=_def.getInput(0))
                else:
                    return LiteralOffset.make_total_unknown_offset()

            elif _def.getOpcode() == PcodeOp.INT_ADD:
                if _def.getInput(0).isConstant() and _def.getInput(1).isConstant():
                    return LiteralOffset.make_constant_offset(
                        _def.getInput(0).getOffset() + _def.getInput(1).getOffset())
                else:
                    return LiteralOffset.make_total_unknown_offset()
            else:
                return LiteralOffset.make_total_unknown_offset()
    ...
    def __call__(self, varnode):
        """
         分析varnode(对应一个offset变量)的def chain，返回其对应的OffsetRepresent实例
         理应在special_int_add call site返回otherPointerOffset，这里交给call site自行处理
         :type varnode Varnode
        """
        if varnode in self.cache:
            return self.cache[varnode]
        else:
            self.cache[varnode] = self.impl(varnode)
            return self.cache[varnode]

AnalysisOffset = OffsetAnalyzer()
