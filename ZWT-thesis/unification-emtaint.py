...
if folder == 'spec-gcc':
    ...
    project_dir = r'/home/jackniu/scripts/emtaints'
else:
    assert False, "unsupportable project"
...
class SolverState(object):
    ...
    def transferArrayToMultStride(self, M):
        ...
        for pointer in M:
            ...
            # 由于do-while循环的存在，数组属性添加的位置可能靠后，例如0x4000+0x4获得0x4这个stride，实际应该加在0x4000
            if prset.need_add_mult_stride():
                for pr in prset:
                    if isinstance(pr, PlusPR) and pr.offset.literal == prset.arrayStride:
                        pr = pr.base
                        while isinstance(pr, PlusPR) and pr.offset.literal == prset.arrayStride:
                            pr = pr.base

                        target_set = self.globalPRs[pr].get_rep()
                        if target_set is not prset:
                            self.globalPRs[pr].isArray = True
                            self.globalPRs[pr].arrayStride = prset.arrayStride
                            prset.isArray = False
                            prset.arrayStride = 0
            ...
            if literal:
                ...
                for ref in refs:
                    if ref.getReferenceType().isWrite():
                        ...
                        op_index = -1
                        if OperandType.isRegister(inst.getOperandType(0)) and OperandType.isAddress(
                                inst.getOperandType(1)):
                            op_index = 0
                        elif OperandType.isRegister(inst.getOperandType(1)) and OperandType.isAddress(
                                inst.getOperandType(0)):
                            op_index = 1

                        if op_index == -1:
                            continue

                        op_refs = inst.getOperandReferences(op_index)
                        for op_ref in op_refs:
                            addr = op_ref.getToAddress()
                            if is_potential_entry_point(addr):
                                res.add(addr.getOffset())
    ...
    def InferPointerVariable(self, func):
        ...
        for op in ops:
            ...
            if opcode == PcodeOp.LOAD:
                ...
                if self.is_special_int_add(op):
                    if output in pointers and input1 in pointers and not AnalysisOffset(op.getInput(0)).is_useless_offset():
                        ptr_related_ops.append(op)
                else:
                    if output in pointers and input0 in pointers and not AnalysisOffset(op.getInput(1)).is_useless_offset():
                        ptr_related_ops.append(op)
    ...
    def buildPDGraph(self, func, M):
        ...
        for pointer in M:
            ...
            if pointer.isConstant():
                dfs(pointer, prset, [])
            elif not sp_visited and pointer.getAddress() == self.sp_address:
                sp_visited = True
                dfs(pointer, prset, [])
...
def is_illegal_for_func_ptr_field(addr_val):
    ...
    对应函数指针的field可能也对应指向数据段的指针, 这也是与not potential_entry逻辑的显著区别。
    也就是说该函数的判定比not potential_entry要松
    空指针0视为合法，理论上之前已经被考虑到了，addr_val取值不会为0
    条件:
    1. 为超过java range的数
    2. 不在程序合法地址范围内
    3. 指向text section，含有getFunctionContaining, 但不是 Entry Point
    """
    if addr_val == 0:
        return False
    ...
    # 指向可执行段 (.text等), 并且不是函数开头
    if mem_block.isExecute() and getFunctionContaining(tgt_addr) is not None and getFunctionAt(tgt_addr) is None:
        return True
