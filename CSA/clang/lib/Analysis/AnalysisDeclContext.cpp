...
void LocationContext::customDumpToStream(raw_ostream &Out,
  ...
  unsigned Frame = 0;
  for (const LocationContext *LCtx = this; LCtx; LCtx = LCtx->getParent()) {
    Out << "{ \"lctx_id\": " << LCtx->getID() << ", \"location_context\": \"";
    switch (LCtx->getKind()) {
    case StackFrame:
      Out << '#' << Frame << " Call\", \"calling\": \"";
      ++Frame;
...
LLVM_DUMP_METHOD void LocationContext::customDump() const { customDumpToStream(llvm::errs()); }
