...
#include "clang/StaticAnalyzer/Core/AnalyzerOptions.def"
  ...
  if (!AnOpts.SummaryDir.empty() && !llvm::sys::fs::is_directory(AnOpts.SummaryDir))
    Diags->Report(diag::err_analyzer_config_invalid_input) << "summary-dir"
                                                           << "a filename";

  if (!AnOpts.LogPath.empty() && !llvm::sys::fs::exists(AnOpts.LogPath))
    Diags->Report(diag::err_analyzer_config_invalid_input) << "log-path"
                                                           << "a filename";
