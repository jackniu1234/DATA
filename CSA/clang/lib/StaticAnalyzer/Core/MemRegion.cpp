...
LLVM_DUMP_METHOD void MemRegion::customDump() const {
  customDumpToStream(llvm::errs());
}
...
void MemRegion::customDumpToStream(raw_ostream &os) const {
  os << "MemRegion[unsupport," << "#kind " << getKind() << "]";
}
...
void FunctionCodeRegion::customDumpToStream(raw_ostream& os) const {
  os << "FunctionCodeRegion[";
  os << getDecl()->getDeclName().getAsString() ;
  if (const FunctionDecl* decl = dyn_cast<FunctionDecl>(getDecl())) {
    os << ',' << decl->getNumParams();

    for (size_t i = 0; i < decl->getNumParams(); i++)
      os << ',' << decl->getParamDecl(i)->getType().getCanonicalType();
  }
  else 
    os << ',' << "null";
...
void ElementRegion::customDumpToStream(raw_ostream &os) const {
  os << "ElementRegion[";
  superRegion->customDumpToStream(os);  // const MemRegion*
  os << ',';
  Index.customDumpToStream(os); // NonLoc
  os << ',' << getElementType().getCanonicalType() << ']';
}
...
void FieldRegion::customDumpToStream(raw_ostream& os) const {
  os << "FieldRegion[";
  superRegion->customDumpToStream(os);
  os << ',';
  if (const FieldDecl* decl = dyn_cast<FieldDecl>(getDecl())) {
    if (const RecordDecl* record = decl->getParent()) {
      // 如果该结构体有名字，则直接输出名字；如果没有名字，则输出所有字段名
      StringRef recordName = decl->getParent()->getName();
      if (!recordName.empty()) {
        os << decl->getParent()->getName();
      }
      else {
        os << '{';
  ...
  llvm_unreachable("please check FieldRegion::customDumpToStream");
}
...
void StringRegion::customDumpToStream(raw_ostream &os) const {
  os << "StringRegion[";
  os << Str->getString().str() << ',' << Str->getKind() << ',' << Str->isPascal() << ','
     << Str->getType().getCanonicalType() << "]";
}
...
void SymbolicRegion::customDumpToStream(raw_ostream &os) const {
  os << "SymbolicRegion[";
  getSuperRegion()->customDumpToStream(os);
  os << ",";
  sym->customDumpToStream(os);
  os << "]";
}
...
void MemSpaceRegion::customDumpToStream(raw_ostream& os) const {
  os << static_cast<const MemRegion *>(this);  // 只输出名字，不输出[]
}
...
void VarRegion::customDumpToStream(raw_ostream& os) const {
  os << "VarRegion[";
  if (auto PVD = dyn_cast<ParmVarDecl>(getDecl())) {
    os << "#argument "  << PVD->getFunctionScopeIndex() << ']';
  }
  else {
    os << "#local";
    // used for understand which variable
    if (const IdentifierInfo *ID = getDecl()->getIdentifier()) {
      os << ' ' << getValueType().getCanonicalType() << ' ' << ID->getName();
    } 
    os << ']';
  }
}
