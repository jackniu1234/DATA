...
LLVM_DUMP_METHOD void Range::dump(raw_ostream &OS) const {
  OS << '[' << From().exportUnsignedValue() << ", " << To().exportUnsignedValue() << ']';
}
// void Range::dumpToStream(raw_ostream &OS) const {
...
void RangeSet::dumpToStream(raw_ostream &OS) const{
  OS << "{ ";
  llvm::interleaveComma(*this, OS, [&OS](const Range &R) { R.dump(OS); });
  OS << " }";
}
...
public:
  ...
  void customDumpToStream(raw_ostream &Out, ProgramStateRef State) const override;

  void customDumpConstraints(raw_ostream &Out,ProgramStateRef State) const;
...
void RangeConstraintManager::customDumpToStream(raw_ostream &Out, ProgramStateRef State) const {
  customDumpConstraints(Out,State);
}
...
void RangeConstraintManager::customDumpConstraints(raw_ostream &Out,ProgramStateRef State) const{
  ConstraintRangeTy Constraints = State->get<ConstraintRange>();
  ...
  for (std::pair<EquivalenceClass, RangeSet> P : Constraints) {
    ...
    for (const SymbolRef &ClassMember : ClassMembers) {
      ...
      std::string S;
      llvm::raw_string_ostream O(S);
      ClassMember->customDumpToStream(O);
      std::tie(std::ignore, insertion_took_place) =
          OrderedConstraints.insert({O.str(), P.second});
      // a bug need to fix
      // assert(insertion_took_place &&
      //        "two symbols should not have the same dump");
    }
  ...
  for (std::pair<std::string, RangeSet> P : OrderedConstraints) {
    ...
    // TODO use customDump instead of <<
    Out << "{ \"symbol\": \"" << P.first << "\", \"range\": \"";
    P.second.dump(Out);
    Out << " " << (*P.second.begin()).From().isUnsigned();
    Out << " " << (*P.second.begin()).From().getBitWidth();
    Out << "\" }";
  }
  Out << "\n";
