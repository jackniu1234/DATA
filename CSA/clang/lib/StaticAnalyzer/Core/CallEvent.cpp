...
StringRef CallEvent::getCalleeName() const {
  const IdentifierInfo* funI = getCalleeIdentifier();
  if (!funI) 
    return StringRef();
  return funI->getName();
}

std::string CallEvent::getCalleeSignature() const {
  const FunctionDecl* FD = dyn_cast_or_null<FunctionDecl>(getDecl());
  if(!FD){
    llvm::outs() << "can't find callee signature! callee name is: " << getCalleeName() << "\n";
    return std::string();
  }
...
void CallEvent::dumpCalleeSignature() const {
  const FunctionDecl* decl = dyn_cast_or_null<FunctionDecl>(getDecl());
    if (decl) {
      llvm::outs() << decl->getReturnType().getAsString();
      llvm::outs() << " ";
      llvm::outs() << getCalleeName();
      llvm::outs() << " (";
      for (size_t i = 0; i < decl->param_size(); i++) {
        QualType parami_type = decl->getParamDecl(i)->getOriginalType();
        if (i != decl->param_size() - 1) {
          llvm::outs() << parami_type.getAsString() << ", ";
        }
        else {
          llvm::outs() << parami_type.getAsString() << ")";
        }
