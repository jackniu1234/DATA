...
namespace {
  struct EvalSummaryApplyContext {
    using CheckersTy = std::vector<CheckerManager::EvalSummaryApplyFunc>;
    
    const CheckersTy &Checkers;
    const CallEvent &Call;
    ExprEngine &Eng;
    std::map<StringRef, void*> CheckerDatasMap;
    
    CheckersTy::const_iterator checkers_begin() { return Checkers.begin(); }
    CheckersTy::const_iterator checkers_end() { return Checkers.end(); }

    EvalSummaryApplyContext(const CheckersTy& checkers,                            
                            const CallEvent& call,
                            ExprEngine& eng,
                            const std::map<StringRef, void*>& checkerDatasMap )
        : Checkers(checkers), Call(call), Eng(eng),
              CheckerDatasMap(checkerDatasMap) {}

    void* getCheckerData(StringRef CheckerName) {
      if (CheckerDatasMap.find(CheckerName) != CheckerDatasMap.end())
        return CheckerDatasMap[CheckerName];
      else
        return nullptr;
    }
    ...
    void runChecker(CheckerManager::EvalSummaryApplyFunc checkFn,
                    NodeBuilder &Bldr, ExplodedNode *Pred) {
      // 此处参照EvalCall设置ProgramPoint
      ProgramPoint L = ProgramPoint::getProgramPoint(
          Call.getOriginExpr(), ProgramPoint::PostStmtKind,
          Pred->getLocationContext(), checkFn.Checker);
      CheckerContext C(Bldr, Eng, Pred, L);
      void* CheckerData = getCheckerData(checkFn.getCheckerBase()->getCheckerName());
      checkFn(*Call.cloneWithState(Pred->getState()), C, CheckerData);
    }
  };
}
...
void CheckerManager::runCheckersForEvalSummaryApply(
                                      ...
                                      const std::map<StringRef, void*>& CheckerDatasMap)
{
  EvalSummaryApplyContext C(EvalSummaryApplyCheckers, Call, Eng, CheckerDatasMap);
  expandGraphWithCheckers(C, Dst, Src);
}
...
void CheckerManager::runCheckersForCustomDumpToStream(raw_ostream &Out, ProgramStateRef State) const {
    Out << "\"checker_messages\": ";
  ...
  NLOut << "\"";  // then begin the next message.
  ...
  for (const auto &CT : CheckerTags) {
    ...
    Out << "{ \"checker\": \"" << CT.second->getCheckerName().getName()<< "\", \"messages\": [" << "\n";
    Out << TempBuf.str();
    Out << "]}";
...
void CheckerManager::_registerForEvalSummaryApply(EvalSummaryApplyFunc checkfn) {
