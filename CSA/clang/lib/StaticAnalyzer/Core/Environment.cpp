...
void Environment::customDumpToStream(raw_ostream &Out, const ASTContext &Ctx,const LocationContext *LCtx) const{
  Out << "\"environment\": ";
  ...
  LCtx->customDumpToStream(Out, [&](const LocationContext *LC) {
    // LCtx items begin
    bool HasItem = false;
    ...
    for (BindingsTy::iterator I = ExprBindings.begin(); I != ExprBindings.end();
      ...
      const Stmt *S = I->first.getStmt();
      Out << "{ \"stmt_id\": " << S->getID(Ctx) << ", \"pretty\": ";
      S->printJson(Out, nullptr, PP, /*AddQuotes=*/true);

      Out << ", \"value\": \"";
      // I->second.printJson(Out, /*AddQuotes=*/true);
      // Out << "\"";
      I->second.customDumpToStream(Out);
