...
// NameTy List = State->get<SummaryDatas>(); State = State->add<SummaryDatas>(E); bool Present = State->contains<SummaryDatas>(E);
REGISTER_LIST_WITH_PROGRAMSTATE(SummaryDatas, MallocCheckerData)
...
public:
  ...
  void evalSummaryApply(const CallEvent& Call, CheckerContext& C, void* Data) const;                   
...
private:
  ...
  mutable std::unique_ptr<BugType> BT_FaultInSummary;
...
#define CHECK_FN(NAME)                                                         \
  ...
  ProgramStateRef summaryMalloc(ProgramStateRef State, CheckerContext& C, 
                                const CallEvent& Call, SVal HeapConjVal, 
                                SVal SizeVal, AllocationFamily Family) const;

  ProgramStateRef summaryFree(ProgramStateRef State, CheckerContext& C,
                              const CallEvent& Call, const Expr* ArgExpr,
                              SVal ArgVal, AllocationFamily Family,
                              bool& has_report) const;

  ProgramStateRef ZeroAllocCheckInSummary(ProgramStateRef State, CheckerContext& C,
                                          const CallEvent& Call, SVal SizeVal,
                                          SVal RetVal) const;
  ...
  /// 该函数用于发现某些不能建模的deallocation函数调用，如果发现，则返回true，并将所有参数的RefState设为Escaped
  ...
  void HandleFaultInSummary(CheckerContext &C, SourceRange Range, StringRef msg) const;
...
ProgramStateRef MallocChecker::summaryMalloc(ProgramStateRef State, CheckerContext& C,
                                             const CallEvent& Call, 
                                             SVal HeapConjVal, 
                                             SVal SizeVal, 
                                             AllocationFamily Family) const {
  ...
  SValBuilder &svalBuilder = C.getSValBuilder();
  State = setDynamicExtent(State, HeapConjVal.getAsRegion(),
                           SizeVal.castAs<DefinedOrUnknownSVal>(), svalBuilder);

  // TODO: 将绑定返回值移动到应该的位置
  State = State->BindExpr(Call.getOriginExpr(), C.getLocationContext(), HeapConjVal);  // 本不应该加在这

  SymbolRef Sym = HeapConjVal.getAsLocSymbol();
  assert(Sym && "HeapConjVal must wrap a symbol");

  State = State->set<RegionState>(Sym, RefState::getAllocated(Family, Call.getOriginExpr()));

  State = ZeroAllocCheckInSummary(State, C, Call, SizeVal, HeapConjVal);
  
  return State;
}

ProgramStateRef MallocChecker::ZeroAllocCheckInSummary(ProgramStateRef State,
                                                       CheckerContext& C,
                                                       const CallEvent& Call,
                                                       SVal SizeVal,
                                                       SVal RetVal) const {
  ...
  QualType UintTy = ACtx.getIntTypeForBitwidth(64, 0);  // 无符号64位整数
  SVal Zero = SVB.makeIntVal(0, UintTy);

  std::tie(TrueState, FalseState) =
      State->assume(SVB.evalEQ(State, SizeVal, Zero)
...
ProgramStateRef MallocChecker::summaryFree(ProgramStateRef State, CheckerContext& C,
                                           const CallEvent& Call, 
  ...
  if (nullState && !notNullState) {
    has_report = true;
    HandleFaultInSummary(C, ArgExpr->getSourceRange(), "null dereference");
    return nullptr;
  }
  ...
  if (ArgVal.isUndef()) {
    has_report = true;
    HandleFaultInSummary(C, ArgExpr->getSourceRange(), "use undefined variable");
    return nullptr;
  }
...
void MallocChecker::evalSummaryApply(const CallEvent& Call,
                                     CheckerContext& C, 
                                     void* Data) const {
  MallocCheckerDatas* D = (MallocCheckerDatas*)Data;
  if (!D)
    return;
  
  ExplodedNode* Pred = C.getPredecessor();
  ProgramStateRef State = C.getState();
  for (MallocCheckerDatas::iterator I = D->begin(), E = D->end(); I != E; ++I) {
    bool has_report = false;

    AllocationFamily Family = (AllocationFamily)I->getFamilyIdx();
    if (I->isMallocData()) {
      ProgramStateRef tmp = summaryMalloc(State, C, Call, I->getHeapConjVal(),
                                          I->getSizeVal(),Family);
      if (tmp) {
        State = tmp;
        Pred = C.addTransition(State, Pred); 
      }
    }
    else if (I->isFreeData()) {
      // 返回nullptr说明该事件不必处理，has_report==true说明发生了错误
      ProgramStateRef tmp = summaryFree(State, C, Call, Call.getOriginExpr(),
                                        I->getFreeVal(), Family, has_report);  
      if (has_report)
        return;
      ...
      llvm_unreachable("not a valid MallocChecker summary item");
    }
  }
}
...
ProgramStateRef MallocChecker::MallocMemAux(CheckerContext &C,
  ...
  State = State->add<SummaryDatas>(
                       MallocCheckerData::CreateMallocData(RetVal, Size, Family));
...
ProgramStateRef MallocChecker::FreeMemAux(
  ...
  if (RsBase) {
    ...
    // 进入该分支的条件：free的arg的base是一个Symbol，并且这个Symbol之前没有见过。
    // 这个Symbol很可能来自参数，但两者不等价，姑且认为等价

    // TODO: 只有当这个Symbol确定是来自参数或者返回值，才进入该分支
    State = State->add<SummaryDatas>(
                       MallocCheckerData::CreateFreeData(ArgVal, Family));
  }
...
void MallocChecker::HandleFaultInSummary(CheckerContext &C, SourceRange Range, 
                                         StringRef msg) const {
  if (ExplodedNode *N = C.generateErrorNode()) {
    if (!BT_FaultInSummary)
      BT_FaultInSummary.reset(new BugType(CheckNames[CK_MallocChecker],
                                          msg,
                                          categories::MemoryError));

    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT_FaultInSummary, msg, N);
    R->addRange(Range);
    C.emitReport(std::move(R));
  }
}
                                     ...
                                     CheckerContext &C) const
{
  ...
  SummaryDatasTy Datas = state->get<SummaryDatas>();
  SummaryDatasTy::Factory &factory = state->get_context<SummaryDatas>();
  SummaryDatasTy newDatas = factory.getEmptyList();
  for (SummaryDatasTy::iterator I = Datas.begin(), E = Datas.end(); I != E; ++I) {
    // 只有malloc类型的summary可能可以舍去; 
    // 因为free类型的summary并不会记录对当前函数malloc出的指针进行的free操作

    auto shouldDelete = [&](SummaryDatasTy::iterator I) {
      if (I->isMallocData()) {
        SymbolRef sym = I->getHeapConjVal().getAsLocSymbol();
        if (sym && SymReaper.isDead(sym))
          return true;
      }
      return false;
    };
    ...
    if (!shouldDelete(I))
      newDatas = factory.add(*I, newDatas);
  }
  // 如果SummaryDatas需要删除部分数据，那么RegionState也一定需要删除部分数据，
  // 所以此处不需要addTransition
  state = state->set<SummaryDatas>(newDatas);
...
void MallocChecker::printState(raw_ostream &Out, ProgramStateRef State,
  ...
  SummaryDatasTy List = State->get<SummaryDatas>();
  if (!List.isEmpty()) {
    for (SummaryDatasTy::iterator I = List.begin(), E = List.end(); I != E; ) {
      Out << "\"";
      I->dumpToStream(Out);
      if (++I == E) 
        Out << "\"" << "\n";
