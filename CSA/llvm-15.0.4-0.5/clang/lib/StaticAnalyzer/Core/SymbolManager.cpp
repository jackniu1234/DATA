...
LLVM_DUMP_METHOD void SymExpr::customDump() const { customDumpToStream(llvm::errs()); }

void SymExpr::customDumpToStream(raw_ostream& os) const { 
  os << "SymExpr[unsupport," << "#kind " << getKind() << "]";
}

template <class LHSTYPE, class RHSTYPE, SymExpr::Kind ClassKind>
void BinarySymExprImpl<LHSTYPE, RHSTYPE, ClassKind>::customDumpToStream(raw_ostream& os) const {
  os << "BinarySymExpr[";
  customDumpToStreamImpl(os, LHS);
  os << "," << BinaryOperator::getOpcodeStr(getOpcode()) << ",";
  customDumpToStreamImpl(os, RHS);
  os << "," << getType().getCanonicalType();
  os << "]";
}

void BinarySymExpr::customDumpToStreamImpl(raw_ostream &os, const SymExpr *Sym) {
  os << "#sym ";
  Sym->customDumpToStream(os);
}

void BinarySymExpr::customDumpToStreamImpl(raw_ostream &os, const llvm::APSInt &Value) {
  os << "#int ";
...
void SymbolConjured::customDumpToStream(raw_ostream& os) const {
  os << "SymbolConjured[" << getSymbolID();
  os << ',' << getType().getCanonicalType() << ']';
}
...
void SymbolRegionValue::customDumpToStream(raw_ostream &os) const {
  // Special handling for argument values.
  if (auto V = dyn_cast<VarRegion>(R)) {
    if (auto PVD = dyn_cast<ParmVarDecl>(V->getDecl())) {
      os << "SymbolRegionValue[" << "#argument "  << PVD->getFunctionScopeIndex() << "]";
      return;
    }
  }
  else {
    os << "SymbolRegionValue[" << "#store ";
