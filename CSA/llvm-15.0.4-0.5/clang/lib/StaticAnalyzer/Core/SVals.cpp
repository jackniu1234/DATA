...
LLVM_DUMP_METHOD void SVal::customDump() const { customDumpToStream(llvm::errs()); }
...
void SVal::dumpToStream(raw_ostream &os) const {
  switch (getBaseKind()) {
    ...
    case UndefinedValKind:
      os << "Undefined";
      break;
    case SummaryFailedValKind:
      os << "SummaryFailed";
      break;
    default:
      llvm_unreachable("maybe SVal::dumpToStream meets SummaryFailedVal");
  }
}

void SVal::customDumpToStream(raw_ostream &os) const {
  switch (getBaseKind()) {
    case UnknownValKind:
      os << "UnknownVal[]";
      break;
    case NonLocKind:
      castAs<NonLoc>().customDumpToStream(os);
      break;
    case LocKind:
      castAs<Loc>().customDumpToStream(os);
      break;
    case UndefinedValKind:
      os << "UndefinedVal[]";
      break;
    case SummaryFailedValKind:
      ...
      break;
    default:
      llvm_unreachable("SVal::customDumpToStream failed");
  }
}
...
void NonLoc::customDumpToStream(raw_ostream& os) const {
  switch (getSubKind()) {
    case nonloc::ConcreteIntKind: {
      os << "ConcreteIntNonLoc[";
      const auto &value = castAs<nonloc::ConcreteInt>().getValue();
      os << value.exportUnsignedValue() << ' ' << (value.isSigned() ? 'S' : 'U')
         << value.getBitWidth() << 'b';
      os << "]";
      break;
    }
    case nonloc::SymbolValKind: {
      os << "SymbolVal[";
      SymbolRef sym = castAs<nonloc::SymbolVal>().getSymbol();
    ...
    case nonloc::LocAsIntegerKind: {
      os << "LocAsInteger[";
      const auto& lai = castAs<nonloc::LocAsInteger>();
      lai.getLoc().customDumpToStream(os);
      os << ',' << lai.getNumBits() << ']';
      break;
    }
    case nonloc::LazyCompoundValKind: {
      os << "LazyCompoundVal[";
      const auto& lcv = castAs<nonloc::LazyCompoundVal>();
      lcv.getRegion()->customDumpToStream(os);
      os << "]";
      break;
    }
    default:
      os << "NonLoc[unsupport," << "#kind " << getSubKind() << "]";
  }
}
...
void Loc::customDumpToStream(raw_ostream& os) const {
  switch (getSubKind()) {
    case loc::MemRegionValKind: {
      os << "MemRegionVal[";
      const MemRegion* R = castAs<loc::MemRegionVal>().getRegion();
      R->customDumpToStream(os);
      os << "]";
      break;
    }
    case loc::ConcreteIntKind: {
      os << "ConcreteIntLoc[";
      const auto& value = castAs<loc::ConcreteInt>().getValue();
      os << value << ' ' << (value.isSigned() ? 'S' : 'U')
         << value.getBitWidth() << 'b';
      os << "]";
      break;
    ...
    default:
      os << "Loc[unsupport," << "#kind " << getSubKind() << "]";
  }
}
