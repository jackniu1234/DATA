...
ExprEngine::ExprEngine(cross_tu::CrossTranslationUnitContext &CTU,
  ...
  JSMgr.loadSummaries(mgr.options.SummaryDir.str());
}
...
void ExprEngine::customDumpToStream(raw_ostream &Out, ProgramStateRef State,const LocationContext *LCtx) const{
  getCheckerManager().runCheckersForCustomDumpToStream(Out,State);
}
...
static bool shouldRemoveDeadBindings(AnalysisManager &AMgr,
  ...
  // TODO: removeDeadBindings需要提前利用摘要是否存在的判断
  // 这条短路路径服务于摘要应用阶段，如果被调用函数可能采用摘要进行处理，
  //  则此时不应进行垃圾清理；采用保守估计，如果被调用函数的函数名位于summaryDir中，
  //  则认为采用摘要进行处理；后续需要转换为利用FunctionDecl的完整信息进行判断
  if (const auto* callStmt = dyn_cast<CallExpr>(S)) {
    if (const FunctionDecl* calleeDecl = callStmt->getDirectCallee()) {
      StringRef calleeName = calleeDecl->getIdentifier()->getName();

      StringRef summaryDir = AMgr.options.SummaryDir;
      std::string path = summaryDir.str() + '/' + calleeName.str() + ".json";
