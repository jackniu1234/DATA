...
ProgramStateRef
ProgramState::invalidateGlobalRegionWhenSummary(Loc LV,
                                                SVal V) const {
  ProgramStateManager &Mgr = getStateManager();
  auto& StMgr = Mgr.StoreMgr;
  
  const MemRegion *R = LV.castAs<loc::MemRegionVal>().getRegion();
  const StoreRef &newStore1 = StMgr->killBinding(getStore(), LV);
  const StoreRef &newStore2 = StMgr->BindDefaultInitial(newStore1.getStore(), R, V);

  ProgramStateRef new_state = makeWithStore(newStore2);
  return new_state;
}
