...
void ExprEngine::evalCall(ExplodedNodeSet &Dst, ExplodedNode *Pred,
  ...
  // 将针对反序列化模块的测试路径挪到外面
  if (Call.getCalleeName() == "callee_ex") {
    std::string path = getSummaryDir().str() + '/' + "callee_ex";
    if (llvm::sys::fs::exists(path)) {
      Reader reader(path);
      shared_ptr<Composer> pComposer = makeOneComposer(*this, Call, Pred);

      doComposeSVal(reader, *pComposer);
      pComposer.reset();
      exit(0);  // 此处结束对反序列化的测试，可以正常退出
    }
  }

  ExplodedNodeSet dstCallEvaluated;

  JsonSummaryManager jsonSummaryManager = this->getJsonSummaryManager();
  JsonSummaryKey key;
  if (callEventMatchSummary(Call, jsonSummaryManager, key)) {
    llvm::outs() << "function: \"" << Call.getCalleeName() << "\" has found summary!\n";
    
    // TODO: 增加摘要存在，但是处理过程失败后转入conservative analysis的逻辑

    for(auto NIT = dstPreVisit.begin(); NIT != dstPreVisit.end(); NIT++){
      ExplodedNode *node = *NIT;
      if (node->isSink()) {
        dstCallEvaluated.Add(node);
        continue;
      }

      shared_ptr<Composer> pComposer = makeOneComposer(*this, Call, node);
      std::vector<StateAndCKSummaryPair> stateAndCKSummaryPairs = 
                            std::move(doComposePaths(Call, jsonSummaryManager.getSummary(key), *pComposer));

      for (const auto& stateAndCKSummaryPair : stateAndCKSummaryPairs) {
        // process path one by one
        ProgramStateRef state = stateAndCKSummaryPair.first;
        const std::map<std::string, void *>& checkerDatasMap =
                                 stateAndCKSummaryPair.second; // std::string->void*

        // add new exploded graph node
        bool isNew;
        ExplodedNode* temp_node = G.getNode(Call.getProgramPoint(), state, false, &isNew);
        temp_node->addPredecessor(node, G);
        if (!isNew)  // this node has already existed, don't preserve it.
          continue;

        // process checker messages
        ExplodedNodeSet tempDstSet;
        ExplodedNodeSet tempSrcSet;
        tempSrcSet.Add(temp_node);

        std::map<StringRef, void*> realCheckerDatasMap;
        for(const auto& item : checkerDatasMap) {
          realCheckerDatasMap[StringRef(item.first)] = item.second;
        }
        
        getCheckerManager().runCheckersForEvalSummaryApply(
                              tempDstSet, tempSrcSet, Call, *this, realCheckerDatasMap);

        // 在此处写入结果
        dstCallEvaluated.insert(tempDstSet);
      }
    }
  }
  else {
