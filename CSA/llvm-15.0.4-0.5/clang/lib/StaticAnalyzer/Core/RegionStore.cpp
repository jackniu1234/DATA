...
private:
  ...
  // store origin region
  const MemRegion * OriginRegion;


  /// Create a key for a binding to region \p r, which has a symbolic offset
  /// from region \p Base.
  explicit BindingKey(const SubRegion *r, const SubRegion *Base, Kind k, const MemRegion *originRegion)
    : P(r, k | Symbolic), Data(reinterpret_cast<uintptr_t>(Base)) , OriginRegion(originRegion) {
  ...
  explicit BindingKey(const MemRegion *r, uint64_t offset, Kind k, const MemRegion *originRegion)
    : P(r, k), Data(offset), OriginRegion(originRegion) {
...
public:
  ...
  void customDumpToStream(raw_ostream &Out) const;
  ...
  LLVM_DUMP_METHOD void customDump() const;
};
uint64_t BindingKey::SharedCounter = 0;
...
BindingKey BindingKey::Make(const MemRegion *R, Kind k) {
  ...
  if (RO.hasSymbolicOffset())
    return BindingKey(cast<SubRegion>(R), cast<SubRegion>(RO.getRegion()), k, R);

  return BindingKey(RO.getRegion(), RO.getOffset(), k , R);
  
}

void BindingKey::customDumpToStream(raw_ostream &Out) const{
  Out << "\"item_region\": ";
  if(this->OriginRegion){
    Out << "\"";
    OriginRegion->customDumpToStream(Out);
    Out << "\"";
  }else{
    Out << "null";
  }
...
static inline raw_ostream &operator<<(raw_ostream &Out, BindingKey K) {
  ...
  if (!K.hasSymbolicOffset())
    ...
    // Out << "null";
    if (K.getRegion()->getKind() == MemRegion::ElementRegionKind){
      const auto *ER = cast<ElementRegion>(K.getRegion());
      ER->getIndex().dumpToStream(Out);
    }else{
      Out << "null";
    }

    // if (K.getConcreteOffsetRegion()->getKind() == MemRegion::SymbolicRegionKind){
    //   // Out << K.getConcreteOffsetRegion()->getKind();
    //   //const SymbolicRegion *SR = cast<SymbolicRegion>(K.getConcreteOffsetRegion());
    //   const auto *SR = cast<SymbolicRegion>(K.getRegion());
    //   SR->dumpToStream(Out);
    // }else{
    //   Out << "null";
    // }
  // debug counter
  // Out << ", \"counter\": " << K.getCounter();
...
void BindingKey::customDump() const {llvm::errs() << *this;}
#endif
...
public:
  ...
  void customDumpToStream(raw_ostream &Out) const {
    for (iterator I = begin(); I != end(); ++I) {
      const ClusterBindings &CB = I.getData();
      // {base: xxx, offset: xxx, binding_kind: xxx,value: xxx}
      for (ClusterBindings::iterator CI = CB.begin(); CI != CB.end(); ++CI) {
        // Out << "{ " << "\"base\": \"" << IK << "\", ";
        // Out << "{ " << "\"base\": \"";
        // IK->customDumpToStream(Out);
        // Out << "\", ";
        std::string str;
        llvm::raw_string_ostream O(str);
        CI.getKey().customDumpToStream(O);
        StringRef ss(str);
        // if(ss.contains(StringRef("#local"))){
        //   if (std::next(I) == end() && std::next(CI) == CB.end()){
        //     Out << "null\n";
        //   }
        //   continue;
        // }
        Out << "{";
        CI.getKey().customDumpToStream(Out);
        Out << ", \"value\": \"";
        // CI.getData().printJson(Out, /*AddQuotes=*/true);
        CI.getData().customDumpToStream(Out);
        Out << "\" }";
        if (std::next(I) != end() || std::next(CI) != CB.end())
           Out << ',';
  ...
  LLVM_DUMP_METHOD void customDump() const { customDumpToStream(llvm::errs()); }
};
} // end anonymous namespace
...
private:
  ...
  /// Whether should clear out the overlap region
  bool ShouldClearOverlapOffset;
...
public:
  RegionStoreManager(ProgramStateManager &mgr)
    ...
    ShouldClearOverlapOffset = Options.ShouldClearOverlapOffset;
  }
...
public: // Part of public interface to class.
  ...
  void iterBindings(Store store, BindingsHandler& f) override {
   ...
   void customDumpToStream(raw_ostream &Out, Store S) const override;
};
...
RegionStoreManager::bind(RegionBindingsConstRef B, Loc L, SVal V) {
  ...
  if(ShouldClearOverlapOffset){
    // Clear out bindings that may overlap with this binding.
    NewB = removeSubRegionBindings(B, cast<SubRegion>(R));
    return NewB.addBinding(BindingKey::Make(R, BindingKey::Direct), V);
  }

  // if have same offset, then remove before one.
  BindingKey TopKey = BindingKey::Make(R, BindingKey::Default);
  const MemRegion *ClusterHead = TopKey.getBaseRegion();
  const ClusterBindings *Cluster = B.lookup(ClusterHead);
  if(Cluster){
    for (ClusterBindings::iterator CI = Cluster->begin(); CI != Cluster->end(); ++CI) {
      if(BindingKey::Make(R, BindingKey::Direct) == CI.getKey()){
        ClusterBindings NewCluster = CBFactory.remove(*Cluster, CI.getKey());
        NewB = NewB.remove(ClusterHead);
        // how to bind cluster to key?
        // add(ClusterHead, NewCluster);
        // how to add new key to this?
        for(ClusterBindings::iterator CII = NewCluster.begin(); CII != NewCluster.end(); ++CII){
          NewB = NewB.addBinding(CII.getKey(),CII.getData());
        }
        // NewB = add(ClusterHead, NewCluster);
        // llvm::outs() << "found you!\n";
        break;
      }
    }
...
void RegionStoreManager::customDumpToStream(raw_ostream &Out, Store S) const {
  RegionBindingsRef Bindings = getRegionBindings(S);
  ...
  // [{base, offset, binding_kind, value}, ...]
  // std::string str;
  // llvm::raw_string_ostream O(str);
  // StringRef ss(str);
  // ss.contains();
  Out << "[" << "\n";
  Bindings.customDumpToStream(Out);
  Out << "]," << "\n";
