...
class ExprInspectionChecker
  ...
  void analyzerCustomDump(const CallExpr *CE, CheckerContext &C) const;
  ...
  void printAndReportForCustomDump(CheckerContext &C, const SVal& What) const;
...
bool ExprInspectionChecker::evalCall(const CallEvent &Call,
  ...
  FnCheck Handler =
      llvm::StringSwitch<FnCheck>(C.getCalleeName(CE))
          ...
          .StartsWith("clang_analyzer_customDump",
                      &ExprInspectionChecker::analyzerCustomDump)
...
void ExprInspectionChecker::printAndReportForCustomDump(CheckerContext &C, const SVal& V) const {
  llvm::SmallString<64> Str;
  llvm::raw_svector_ostream OS(Str);
  V.customDumpToStream(OS);
  reportBug(OS.str(), C);
}
...
void ExprInspectionChecker::analyzerCustomDump(const CallExpr *CE,
                                               CheckerContext &C) const {
  const Expr *Arg = getArgExpr(CE, C);
  if (!Arg)
    return;
  ...
  SVal V = C.getSVal(Arg);
  printAndReportForCustomDump(C, V);
}
