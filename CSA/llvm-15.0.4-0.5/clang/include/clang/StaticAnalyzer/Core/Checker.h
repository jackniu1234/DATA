...
class SummaryApply {
  template <typename CHECKER>
  static void _evalSummaryApply(void *checker,
                                const CallEvent &Call,
                                CheckerContext &C, 
                                void* Data) {
    return ((const CHECKER *)checker)->evalSummaryApply(Call, C, Data);
  }

public:
  template <typename CHECKER>
  static void _register(CHECKER *checker, CheckerManager &mgr) {
    // 最外层函数调用的参数是一个EvalSummaryApplyFunc的构造函数
...
public:
  ...
  StringRef getStringRefName() const;
