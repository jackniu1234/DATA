...
#include "clang/StaticAnalyzer/Core/PathSensitive/SVals.def"
  ...
  enum { BaseBits = 3, BaseMask = 0b111 };

protected:
  const void *Data = nullptr;

  /// The lowest 2 bits are a BaseKind (0 -- 3).  (现在是最低三位,0-4)
...
public:
  ...
  bool isSummaryFailed() const {
    return getRawKind() == SummaryFailedValKind;
  }
  ...
  void customDump() const;
  void customDumpToStream(raw_ostream &OS) const;
...
class SummaryFailedVal : public SVal {
public:
  SummaryFailedVal() : SVal(SummaryFailedValKind) {}
  static bool classof(SVal V) { return V.getBaseKind() == SummaryFailedValKind; }
};
