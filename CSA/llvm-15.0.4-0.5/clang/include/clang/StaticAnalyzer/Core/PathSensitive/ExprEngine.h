...
public:
  ...
  // 获得存放摘要的文件夹路径，目前被写死为/bigdata/huawei-proj/jackniu/summaries
  StringRef getSummaryDir() { return getAnalysisManager().options.SummaryDir; }
  // 获得logging file的路径
  StringRef getLogPath() { return getAnalysisManager().options.LogPath; }
  // 用来决定使用customDump方法或dump方法，打印符号值
  bool shouldCustomDump() { return getAnalysisManager().options.ShouldCustomDump; }
  ...
  JsonSummaryManager &getJsonSummaryManager() { 
    return JSMgr; 
  }
  ...
  void customDumpToStream(raw_ostream &Out, ProgramStateRef State,const LocationContext *LCtx) const;
...
public:
  ...
  llvm::Optional<CFGElement> getNextCFGElement() {
    auto block = *currBldrCtx->getBlock();
    if (currStmtIdx + 1 < block.size())
      return block[currStmtIdx+1];
    else
      return llvm::None;
  }
